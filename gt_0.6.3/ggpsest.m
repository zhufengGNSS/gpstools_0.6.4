...
% [note]   :
% [version]: $Revision: 33 $ $Date: 06/07/21 18:04 $
...
gut('newprms','prm3',[384,78,210,17],prm3);
set(gut('newtext','pmsg',[164,50,428,20],'',2),'foregroundcolor','b');
gut('newprog','prog',[164,34,428,15],[0,0.7,1]);
...
prm.dirs.est=gfilepath('',prm.dirs.est,prm.tstart);
gut('setudata','gtest',prm);
...
pats={'prm_*.mat','settings (prm_*.mat)';'prm_*.m','settings (prm_*.m)'};
if save
...
[file,p]=uigetfile({'prm_*.mat','settings (prm_*.mat)'},'Load Settings',path);
...
'p','Repair Steered Clock Jump',        sel1
'p','Repair Cycle Slip',                sel1
'n','# of Points for Poly. Fitting',    10
'n','Degrees for Poly. Fitting',        2
'p','Separate Arc at Eclipse In/Out',   sel1
'p','Phase Multipath Correction',       sel1
's','Phase MP Profile Start/End (days)',[0,0]
'n','Adjustment of Sidereal Day (sec)', 0
};
prm3={
' ','Code Multipath Profile',''
'f','',''
};
diss={'prm1_3','prm2_9','prm2_10_1_1','prm2_10_1_2','prm2_11','_prm1_3',...
      '_prm2_9','_prm2_10','_prm2_11','prm3_1','_prm3_1'}; 
...
gut('setdis',diss);
prm=gut('getudata','gtest');
gut('setprms','prm1',prm.f1,prm.f2,prm.L1code,prm.obs.elmin,prm.obs.gapmax,...
    prm.obs.arcmin,prm.obs.pntmin,prm.obs.sigmw,prm.obs.sigif,prm.obs.dgmax,...
    prm.obs.dwl,prm.obs.dmp1,prm.obs.dmp2,prm.obs.outl)
gut('setprms','prm2',prm.obs.wind(1),prm.obs.wind(2),prm.obs.elwe,...
    prm.obs.clkrep,prm.obs.reps,prm.obs.npnt,prm.obs.nmax,prm.obs.separc,...
    prm.obs.srfilt,prm.obs.srdays,prm.obs.sradj)
gut('setprms','prm3','',prm.mpc,'')
gut('newokcancelbtn','',[334,4,180,22]);
if ~gut('waitok'), return, end
prm.rcvs=gut('getstring','rcvs');
[prm.f1,prm.f2,prm.L1code,prm.obs.elmin,prm.obs.gapmax,prm.obs.arcmin,...
 prm.obs.pntmin,prm.obs.sigmw,prm.obs.sigif,prm.obs.dgmax,prm.obs.dwl,...
 prm.obs.dmp1,prm.obs.dmp2,prm.obs.outl]=gut('getprms','prm1');
[prm.obs.wind(1),prm.obs.wind(2),prm.obs.elwe,prm.obs.clkrep,prm.obs.reps,...
 prm.obs.npnt,prm.obs.nmax,prm.obs.separc,prm.obs.srfilt,prm.obs.srdays,...
 prm.obs.sradj]=gut('getprms','prm2');
...
sel3={'','<estimated>','IGS Final','IGS Rapid','IGS URapid','Est(forward)','Est(backward)','Est(smoothed)','IGS URapid(Pred)','CODE','EMR','ESA','GFZ','JPL','MIT','IGS/CODE','CODE Rapid','IGR/CODER'};
sel4={'','<estimated>','IGS Final','ITRF2000','ITRF97','GSIPOS','Est(forward)','Est(backward)','Est(smoothed)','IGS00','IGb00','Parameter File'};
...
for n=1:6
    ...
    gut('newpopm',['satc_',num2str(n)],[203,y,95,23],sel3,0:17,'');
    gut('newpopm',['satr_',num2str(n)],[298,y,95,23],sel1,0:1,'');
end
for n=1:8
    y=221-n*24;
    gut('newpopm',['rcvn_',num2str(n)],[  8,y,95,23],rcvs,{},'');
    gut('newpopm',['rcvp_',num2str(n)],[108,y,95,23],sel4,0:11,'');
    gut('newpopm',['rcvc_',num2str(n)],[203,y,95,23],sel3,0:15,'');
...
function cbEst5
rcvs=gut('getstring','rcvs');
sel1={{'Saastamoinen'},{'trop_saast'}};
sel2={{'COSZ','NMF','GMF'},{'mapf_cosz','mapf_nmf','mapf_gmf'}};
sel3={{'OFF','ON'},[0,1]};
sel4={{'Standard Kalman'},{'filterekf'}};
sel6={{'IAU1976/1980','IERS1996'},{'','iers1996'}};
sel7={{'Standard Atmos.','JMA GSM Online','JMA MSM Online'},{'','gso','mso'}};
...
set(gut('newbtn','',[158,6,16,18],'?',[mfilename,' cbPathHelp']),'fontsize',8);
...
str2={': Sat/Rcv Name (UPPER Case)',': Sat/Rcv Name (lower Case)',...
      ...
      ': Day of Week (d)',': GPS Week No (wwww)',': Install Path'};
gut('newdlg','','Keyword Replacements',[220,218]);
gut('newtextv','',[20,30,30,16],str1);
gut('newtextv','',[50,30,200,16],str2);
gut('newokbtn','',[135,4,80,23]);
gut('waitok');
close;
...
log={}; dirs=gfilepath(prm.dirs.est,'',prm.tstart);
