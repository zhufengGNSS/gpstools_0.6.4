...
%            phs   = phase windup correction (rad)
%           (gg)   = detailed model terms (for debug)
% [note]   :
% [version]: $Revision: 23 $ $Date: 06/07/14 15:08 $
...
static int IxIndex(const mxArray *ix, const char *f, int m, int n, int *nx)
{
	...
	*nx=0;
	...
	if (mxIsCell(p)) {
		if (mxGetM(p)<=m||mxGetN(p)<=n||!(p=mxGetCell(p,m+mxGetM(p)*n)))
			return 0;
	}
...
static int RangeModel(const double *td, const double *ts, const double *x,
				      ...
				      double *phs, double *drds, double *gg)
{
	int i,j,s,r,n,m;
	...
	if (azel[1]<p->elmin[0]||p->elmax[0]<azel[1]) return 0;
	...
	if (strcmp(p->mapf,"mapf_nmf")==0)
		mapf_nmf(&t,azel,gpos,&mfd,&mfw);
	else if (strcmp(p->mapf,"mapf_gmf")==0) {
	...
	*g=range+cdtr-cdts+mfd*trd+mfw*trw+N;
	...
	if ((int)p->obscorr[0]&&p->ants) {
		p1=p->ants+19*s;
		c_apcs=-SatApc(rsat,rrcv,p->rsun,p1,p1+3);
	}
	if ((int)p->obscorr[1]&&p->antp) {
		p1=p->antp+47*r;
		RcvApc(azel,p1,p1+3,p1+6,p1+9,p1+28,ap);
		c_apcr=-(p->cif[0]*ap[0]+p->cif[1]*ap[1]);
	}
	if ((int)p->obscorr[2]) c_rel=RelCorr(rsat,stas+3,rrcv);
	if ((int)p->obscorr[3]) {
		p1=phs+s+p->ns*r;
		*p1=phwindup(rsat,p->rsun,posr,p->U,p1,p->rmode);
		c_phw=M_C*(p->cif[0]*(*p1)/p->f1[0]+p->cif[1]*(*p1)/p->f2[0])/2.0/M_PI;
	}
				      ...
				      double *phs, double *drds, double *index, double *gg)
{
	...
	for (i=n=0;i<nz;i++) {
		...
		if (RangeModel(td,ts,x,nx,ix,izn,p,g+n,G+nx*n,azel+2*n,phs,drds+6*n,
					   gg?gg+15*i:NULL))
			index[n++]=(double)(i+1);
	}
	return n;
