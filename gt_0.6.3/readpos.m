...
% [note]   :
% [version]: $Revision: 29 $ $Date: 06/07/08 14:21 $
...
elseif strcmp(possrc,'gsipos')
    for m=1:length(rcvs)
        for n=1:length(time)
            [poss(n,:,m),covs(n,:,m)]=ReadPosData(td,time(n),rcvs{m},posdir,src);
        end
    end
else
    for n=1:length(time)
...
function [pos,cov]=ReadPosData(td,ts,rcv,posdir,possrc)
...
case 'igssnx', % IGS Final
    if td~=td_|~strcmp(posdir,dir_)|~strcmp(possrc,src_)
        ...
        if isempty(names)
            gt_log('no position data        : %s file=%s src=%s',rcv,file,possrc);
            return
        end
    end
...
case 'gsipos', % GSI position estimation
    if td~=td_|~strcmp(rcv,rcv_)|~strcmp(posdir,dir_)|~strcmp(possrc,src_)
...
case {'itrf2000','itrf97','igs00','igb00'}, % ITRF2000/97,IGS00,IGb00
    switch possrc
    case 'itrf2000'
    ...
    if ~strcmp(posdir,dir_)|~strcmp(possrc,src_)
...
while 1
    ...
    if ~isempty(StrToNum(str,1,1))
        name=SubStr(str,34,4);
        if strcmp(name,'    ')
            if n>0
                for m=1:3
                    vels(m,n)=StrToNum(str,26+m*13,12);
                    vsigs(m,n)=StrToNum(str,72+m*6,5);
                end
            end
        elseif ~isempty(name)
            n=n+1;
            names{n}=name;
            for m=1:3
                poss(m,n)=StrToNum(str,26+m*13,12);
                psigs(m,n)=StrToNum(str,72+m*6,5);
            end
        end
    end
end
