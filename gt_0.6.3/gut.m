...
%              'newtime',tag,pos,time        : generate date/time input
...
% [argout] : argouts
% [note]   :
% [version]: $Revision: 41 $ $Date: 06/07/21 18:02 $
...
global p_gut
h=uicontrol('style',style,'tag',tag,'position',pos,'fontname',p_gut.ufont,...
            'fontsize',p_gut.usize,'handlevisibility','off',varargin{:});
...
h=geth(tag);
if get(h,'max')>1
    set(h,'string',labels,'userdata',vals,'value',[]);
else
    if isempty(labels), labels={''}; end
    set(h,'string',labels,'userdata',vals,'value',1);
end
...
% date/time input --------------------------------------------------------------
function h=newtime(tag,pos,time)
x=[0,0,0,120,147,174]; pos(1)=pos(1)+min(pos(3)-200,0);
h=newymd(tag,pos+[0,0,60,0],time,'');
for n=4:6, newedit([tag,'_',num2str(n)],[pos(1)+x(n),pos(2)-2,26,22],0); end
settime(tag,time);
...
setymd(tag,time); if length(time)<6, time(4:6)=0; end
...
ext={}; time={};
for n=1:length(f)
    [s,ext{n}]=strtok(f(n).name,'.'); time{n}=datefmt(f(n).date);
end
...
figure(h)
set(geth(tag),'string',str,'value',[],'userdata',f)
set(h,'pointer','arrow')
...
drv={'a:\'};
for d='b':'z', if ~isempty(dir([d,':\'])), drv={drv{:},[d,':\']}; end, end
ds=drv;
...
if isempty(file), file='*.*'; end
[file,path]=uigetfile(file,'Select File');
if file~=0, set(get(gcbo,'userdata'),'string',fullfile(path,file)), end
...
p=get(gcf,'position'); p(3:4)=max(p(3:4),[100,100]);
...
function cbViewSave
[file,path]=uiputfile('*.*','Save To Text File'); if file==0, return, end
f=fopen([path,file],'wt'); if f==-1, return, end
for s=getstring('vstr')', fprintf(f,'%s\n',s{1}); end
fclose(f);
...
dirs=gfilepath('',dirs);
if isempty(dirs), if isempty(hist{1}), dirs=pwd; else dirs=hist{1}; end, end
