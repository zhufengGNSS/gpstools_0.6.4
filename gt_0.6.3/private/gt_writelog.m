...
% [argout] : none
% [note]   :
% [version]: $Revision: 9 $ $Date: 06/07/21 18:54 $
...
if ~isempty(obss), writeobss(f,td,obss,prm); end
if isfield(stats,'ecls'), writeecls(f,td,stats.ecls,prm); end
...
satclk={'-','<estimated>','IGS final','IGS rapid','IGS urapid','est(forwd)','est(back) ','est(smooth)','IGS pred','COD','EMR','ESA','GFZ','JPL','MIT','IGS/COD','COD rapid','IGR/CODR'};
satsrp={'-','<estimated>'};
rcvpos={'-','<estimated>','IGS00  ','ITRF2000 ','ITRF97 ','GSI    ','est(forwd)','est(back) ','est(smooth)','IGS00  ','IGb00  ','prmfile'};
...
erpsrc={'-','<estimated>','IGS final','IGS urapid','IERS bulB','IERS bulA','IERS C04',...
...
if ~isempty(logs)
    fprintf(f,'\n');
    fprintf(f,' rcv             time         jump(ms) time-tag offset\n');
    fprintl(f,logc);
end
fprintf(f,'\n');

% write receiver info/eclipse conditions ---------------------------------------
function writeobss(f,td,stats,prm)
if isempty(stats), return, end
rcvprm=prm_gpsrcvs;
...
% write eclipse conditions -----------------------------------------------------
function writeecls(f,td,stats,prm)
fprintf(f,'satellite eclipse periods :\n\n');
for n=1:size(stats,1)
    ts=mjdtocal(td,stats(n,2)); te=mjdtocal(td,stats(n,3));
    fprintf(f,'%-7s : %s\n',prm.sats{stats(n,1)},pstr(ts,te));
end
fprintf(f,'\n');
