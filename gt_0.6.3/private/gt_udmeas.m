...
% [note]   :
% [version]: $Revision: 7 $ $Date: 06/07/08 1:16 $
...
% set default meteorological parameters
if isempty(state.metr)
    for n=1:length(prm.rcvs)
        gpos=eceftogeod(state.posr(:,n));
        state.metr(:,n)=[prm.metprm(1)*(1-2.2557E-5*gpos(3))^5.2568;...
                         prm.metprm(2)-6.5E-3*gpos(3);prm.metprm(3)];
    end
end
...
% filter
[x,P,phs,vz,res,ds]=filt(td,t,x,ix,P,z,tz,iz,antp,state,phs,pass,prm);
...
% measurement model ------------------------------------------------------------
function [v,H,R,phs,i,ds]=measmodel(td,t,x,ix,z,tz,iz,antp,state,phs,prm)
...
[y,H,azel,phs,drds,j,ds(i,3:end)]=rangemodel(td,t,x,ix,[tz(i),double(iz(i,:))],...
