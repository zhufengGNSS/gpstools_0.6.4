...
%
% [version]: $Revision: 108 $ $Date: 06/07/21 1:58 $
...
if prm.tunit<=0|168<prm.tunit, msg='unit time out of range'; return, end
if prm.tint<1|86400<prm.tint, msg='estimation interval out of range'; return, end
if prm.tover<0|prm.tunit<prm.tover, msg='overlap time out of range'; return, end
...
for t=(floor(ts/tu):floor(te/tu))*tu
    ...
    if ~prm.dbout
        try
            ...
            status=-1; msg=lasterr;
            p=findstr(msg,'@'); if ~isempty(p), msg=msg(p+1:end); end
            gt_log('aborted by error : %s',msg);
        end
    else
        dbstop if error;
        [status,msg,s]=execest(td,time,prm,ver,s);
...
[t,i,j]=intersect(time,round(tz)); j=[1,j+1];

for pasn=1:10
    pack; xs=repmat(nan,length(time),length(x)); ps=xs;
    ...
    for n=nn
        ...
        [x,P,arc,phs,iz(k,3),res(k,:),iv,ds]=gt_udmeas(td,t(n),x,ix,P,arc,...
            phs,z(k,:),tz(k),iz(k,:),antp,state(i(n)),sig,[pass,pasn],fb,prm);
    ...
    [xs,xz,xa,stats]=gt_qc(td,time,xs,ps,ix,tz,iz,res,arc,prm,pass,abort);
...
if ~abort
    saveest(td,time,ix,xs,ps,arc,fb,prm);
    saveres(td,time(1),tz,iz,res,fb,prm);
end
if ~isempty(dbg), savedbg(td,tz,iz,dbg,fb,prm); end
...
function saveest(td,time,ix,xs,ps,arc,fb,prm)
tu=prm.tunit*3600; to=prm.tover*3600; t=floor((time(1)+to)/tu)*tu;
epoch=mjdtocal(td,t); time=time(:)-t;
arc(:,1:2)=arc(:,1:2)-t;
if findstr(prm.dirs.est,'%r')
    prm.dirs.est=strrep(prm.dirs.est,'%r',prm.rcvs{1});
end
...
dirs=strrep(prm.dirs.est,'%r',prm.rcvs{1});
file=sprintf('dbg%s_%04d%02d%02d%02d%02d%02.0f.mat',s(fb),epoch);
file=gfilepath(dirs,file,prm.tstart,'',1);
gmsg('saving : %s',file);
