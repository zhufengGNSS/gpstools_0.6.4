...
%            08/11/21  0.4  suppress warning on overwriting array (gt_0.6.4)
...
if selrcv
    plist=prm_gpsrcvs; grp=unique(plist(:,2));
else
    plist=prm_gpssats; grp={''};
end
set(h,'userdata',plist);
gut('newtext','text1',[10,375,230,20],'',2);
gut('newpopm','grp', [301,376,228,23],grp,{},[mfilename,' cbChgGrp']);
if selrcv, gut('setsel','grp',plist{1,2}); end
...
patt={'*.txt','text (*.txt)';'*.mat','settings (prm_*.mat)';'*.*','all (*.*)'};
