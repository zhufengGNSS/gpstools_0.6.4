...
%            08/11/26   0.9  fix bug on reading broadcast ephemeris (gt_0.6.4)
%                            fix bug on re-reading missing ephemeris
%                            suppress warning of interp1
%-------------------------------------------------------------------------------
...
case 'ephs'
    ...
    if ~strcmp([d,f],file)|~strcmp([sats{:}],[esats{:}])
        [epoch,time,ephs_,esats_,sigs_]=ReadEphs(d,f,sats);
        if isempty(epoch), return, end
        file=[d,f]; ephs=ephs_; esats=esats_; sigs=sigs_;
        te=caltomjd(epoch(1:3)); ts=epoch(4)*3600+epoch(5)*60+epoch(6);
        ti=(time(end)-time(1))/(length(time)-1);
    end

otherwise % sp3
    if any(strcmp(ephsrc,{'igu','igp'})) % sp3 ultra-rapid
    ...
    if ~strcmp(f,file)
        [epoch,time,ephs_,esats_,accs,sigs_]=readsp3(f);
        if isempty(epoch)
            gt_log('no sp3 ephemeris        : %s',f);
            return
        end
        file=f; ephs=ephs_; esats=esats_; sigs=sigs_;
...
if td+time(1)/86400<ts(1)-0.01|ts(2)+1.01<td+time(end)/86400
    ssats={}; for n=1:99, ssats={ssats{:},sprintf('GPS%02d',n)}; end
