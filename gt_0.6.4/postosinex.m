function postosinex(td,span,tint,rcvs,indir,outdir,fb,tunit,trprm,cbmsg)
%-------------------------------------------------------------------------------
% [system] : GpsTools
...
%            08/12/05  0.2  fix bug on day of year (gt_0.6.4)
...
if nargin<10, cbmsg=''; end

ver=gpstools('version');

switch fb
case 'f',  time=(1:span)*86400-tint;
case 'b',  time=(0:span-1)*86400;
otherwise, time=(0:span-1)*86400+tunit*3600/2;
end

for n=1:length(time)
    % read position estimation results
    [poss,covs]=readpos(td,time(n),rcvs,indir,['pos',fb],tunit);
    
    msg=sprintf('%04d/%02d/%02d %02d:%02d%*d',mjdtocal(td,time(n))); 
    
    if ~isempty(poss)
        if outmsg(cbmsg,['generating sinex: ',msg]), return; end
        
        % trasformation correction
        [poss,tr]=TrCorr(td+time(n)/86400,poss,rcvs,trprm);
        
        % save sinex file
        SaveSinex(td,time(n),poss(1,:,:),covs(1,:,:),rcvs,indir,outdir,fb,tr,...
                  trprm,ver)
    else
        if outmsg(cbmsg,['no position data: ',msg]), return; end
    end
end
...
dv=mjdtocal(td,time);
file=sprintf('pos%s%04d%02d%02d.snx',fb,dv(1:3));
file=gfilepath(outdir,file,dv,'',1);
f=fopen(file,'wt');
...
fprintf(f,'%%=SNX %4.2f %-3.3s %s %-3.3s %s %s P %05d 2 S           \n',...
        ...
        sinext(td+time/86400),sinext(td+time/86400),length(rcvs));
fprintf(f,'*-------------------------------------------------------------------------------\n');
...
rprm=prm_gpsrcvs;
rpos=readpos(td,time,rcvs,'','approx');
for n=1:length(rcvs)
    ...
    if all(~isnan(poss(1,:,n)))
        for m=1:3
            fprintf(f,' %5d %-6.6s %-4.4s  A ---- %s %-4.4s 2 %s %s\n',...
                    index,prm{m},rcvs{n}(end-3:end),sinext(td+time/86400),'m',...
...
if ~isempty(tr)
    ...
    for n=1:7
        fprintf(f,' %5d %-6.6s %-4.4s  A ---- %s %-4.4s 2 %s %s\n',...
                n,prm{n},'----',sinext(floor(td+time/86400)),units{n},...
