...
%            06/12/08  0.3  fix bug on reading satellite parameter (gt_0.6.3p2)
%            06/12/25  0.4  fix bug on replacing keyword in in/out directories (gt_0.6.3p4)
%            07/01/06  0.5  fix bug on saving files with overlap time > 0 (gt_0.6.3p4)
%            08/11/25  0.3  support L1/L2 satellite antenna correction (gt_0.6.4)
...
ts=time(1)+prm.tover*3600;
dd=floor(ts/86400);
td=td+dd; time=time-86400*dd;
tstart=mjdtocal(td,ts);
...
if ~isempty(prm.dirs.obc), prm.dirs.obc =gfilepath('',prm.dirs.obc, []); else prm.dirs.obc =prm.dirs.est; end
...
% replace keyword for output directory (%O)
prm.dirs.obc =strrep(prm.dirs.obc, '%O',prm.dirs.est);
prm.dirs.obs =strrep(prm.dirs.obs, '%O',prm.dirs.est);
prm.dirs.nav =strrep(prm.dirs.nav, '%O',prm.dirs.est);
prm.dirs.eph =strrep(prm.dirs.eph, '%O',prm.dirs.est);
prm.dirs.clk =strrep(prm.dirs.clk, '%O',prm.dirs.est);
prm.dirs.pos =strrep(prm.dirs.pos, '%O',prm.dirs.est);
prm.dirs.erp =strrep(prm.dirs.erp, '%O',prm.dirs.est);
prm.dirs.trop=strrep(prm.dirs.trop,'%O',prm.dirs.est);
prm.dirs.ion =strrep(prm.dirs.ion, '%O',prm.dirs.est);
prm.dirs.inp =strrep(prm.dirs.inp, '%O',prm.dirs.est);
prm.dirs.inpr=strrep(prm.dirs.inpr,'%O',prm.dirs.est);
prm.satsrpf  =strrep(prm.satsrpf,  '%O',prm.dirs.est);
prm.rcvposf  =strrep(prm.rcvposf,  '%O',prm.dirs.est);
prm.pcv      =strrep(prm.pcv,      '%O',prm.dirs.est);
prm.satpcv   =strrep(prm.satpcv,   '%O',prm.dirs.est);
prm.oload    =strrep(prm.oload,    '%O',prm.dirs.est);
prm.mpc      =strrep(prm.mpc,      '%O',prm.dirs.est);
prm.mpp      =strrep(prm.mpp,      '%O',prm.dirs.est);
prm.exsatrcv =strrep(prm.exsatrcv, '%O',prm.dirs.est);

% read satellite parameters
prm.satprm=prm_gpssats(td+ts/86400);
...
prm.ants=zeros(38,length(prm.sats));
for n=1:length(prm.sats)
    ...
    if strcmp(pcvext,'.atx') % antex pcv file
        [apc1,apc2,apv1,apv2,stat]=readpcv('',prm.sats{n},prm.satpcv,td+ts/86400);
        if stat
            prm.ants(:,n)=[apc1(:)',apc2(:)',apv1(1,1:16),apv2(1,1:16)]';
        else
            dv=mjdtocal(td+ts/86400);
            gt_log('no satellite antenna pcv: %s file=%s %04d/%02d/%02d',prm.sats{n},...
                   prm.satpcv,dv(1:3));
        end
    else
        i=find(strcmp(prm.sats{n},prm.satprm(:,1)));
        if ~isempty(i)
            if prm.satprm{i,2}==0 % Block II/IIA
                prm.ants(1:3,n)=[0.279;0.000;1.023];
                prm.ants(4:6,n)=[0.279;0.000;1.023];
            end
        else
...
if prm.est.erp>=1&~isempty(prm.initerp)
    prm.erp=readerp(tutc(td,ts),prm.dirs.erp,prm.initerp);
else
    prm.erp=[0,0,0,0,0];
end
