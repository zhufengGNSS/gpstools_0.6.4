...
%                state(n).mfcr(:,r) = time(n) prm.rcvs{r} mappfint function coef.
...
%                                     (for no gpstime-aligned precise clock)
%            abort   = abort status (1:aborted,0:completed,-1:error)
%            msg     = error message
%            stats   = satellite unhealthy/eclipse status
% [note]   :
...
%            08/11/21  0.3  support igs30,cod5s,igscod5s for sat clock
%                           support itrf2005,igs05 for rcv position
%                           support gpt for meteorological parameters
%                           add satellite unhealthy log
%                           add mappfing function coef. in state
%                           time of position state at prm.tpos+0.5d
%                           fix bug on estimation of satellite orbit/clock
%-------------------------------------------------------------------------------
...
srcclk={'','igs','igr','igu','clkf','clkb','clkfb','igp','cod','emr','esa','gfz','jpl','mit','igscod','codr','igrcodr','igs30s','cod5s','igscod5s'};
srcerp={'','igs','igr','bulb','bula','c04','erpf','erpb','erpfb','cod','emr','esa','gfz','jpl','mit'};
nt=length(time); ns=length(prm.sats);

s.ephs=repmat(nan,6,ns);
s.clks=repmat(nan,2,ns);
s.ephv=[]; s.clkv=[];
if prm.sdfact2>0
    if any(prm.est.sato>=2), s.ephv=repmat(nan,6,ns); end
    if any(prm.est.satc>=2), s.clkv=repmat(nan,2,ns); end
end
...
% satellite unhealthy status
for n=1:ns
    navs=nav(inav==n,:); fsvh=0;
    for m=1:nt
        [pos,dts,vel,svh]=navtostate(td,time(m),navs);
        if svh==0 % healthy satellite
            state(m).ephs(:,n)=[pos;vel];
            state(m).clks(:,n)=dts(1:2);
            fsvh=0;
        elseif fsvh==0
            stats=[stats;n,td,time(m),time(m),1]; fsvh=1;
        else
            stats(end,4)=time(m);
        end
    end
...
for n=2:length(srcclk) % satellite clock
    ...
    if ~isempty(i)
        ...
        for m=1:nt
            ...
            state(m).clks(1,i)=clk(m,1,:);
            if prm.sdfact2>0, state(m).clkv(1,i)=sig(m,1,:).^2; end
        end
    end
end
...
for n=1:ns
    ...
    stats=[stats;repmat([n,td],length(i),1),time(i)',time(j)',repmat(2,length(i),1)];
end
...
srcpos={'','igssnx','itrf2000','itrf97','gsipos','posf','posb','posfb','igs00','igb00','igs05','itrf2005'};
...
for n=1:nt
    ...
    if any(strcmp(prm.metsrc,{'gso','mso'}))
        state(n).metr=repmat(nan,3,nr);
    else
        state(n).metr=[];
    end
    state(n).mfcr=[];
...
for n=2:length(srcpos) % station position
    ...
    if ~isempty(i)
        ...
        if isempty(prm.tp)
            ...
            [pos,cov]=readpos(prm.tp+0.5,0,prm.rcvs(i),dirs,srcpos{n},prm.tunit,'interp');
            for m=1:nt
...
if any(strcmp(prm.metsrc,{'gso','mso'})) % station meteorological parameters
