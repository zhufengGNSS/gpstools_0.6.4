...
%            08/11/19  0.3  output nan if time-diff>MAXDTOE or SVH!=0 (gt_0.6.3p6)
...
#define MAXDTOE		10800.0		/* max time-difference referenced to TOE */
				      ...
				      char type, int opt, double *pos, double *dts, double *vel)
{
	...
	if (type=='N') {
		...
		for (i=0;i<nnav;i++) {
			...
			if (!svh) svh=nav[i+30*nnav];
			if (ABS(dt)<ABS(dtmin)) {dtmin=dt; imin=i;}
		}
		if (imin<0) {
			for (i=0;i<3;i++) {
				pos[i]=mxGetNaN();
				if (dts) dts[i]=mxGetNaN();
