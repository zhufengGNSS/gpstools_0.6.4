...
%            08/11/21  0.7  not use ephemeris if time-diff>MAXDTOE or SVH!=0 (gt_0.6.3p6)
%            08/11/21  0.8  add argin opt (gt_0.6.4)
%-----------------------------------------------------------------------------*/
#include "qtcmn.h"
#include "mex.h"
...
#define	MAXDTOE		10800.0			/* max difference to TOE (s) */
...
static int residual(const double *td, const double *ts, const double *z,
					...
					const double *cdtr, const double *dflg, const double *opt,
					double *dz, double *H)
{
	double gpos[3],poss[3],posz[3],dts[3],r[3],azel[2],navi[37],R[9];
	double range,rng,trop=0.0,dt,dtmin;
	int i,j,imin,n=0,svh;
	...
	for (i=0;i<nz;i++) {
		...
		for (j=svh=0,imin=-1,dtmin=99999999.0;j<nnav;j++) {
			if ((int)inav[j]!=(int)iz[i]) continue;
			dt=(td[0]-44244.0-nav[j+27*nnav]*7)*86400.0+ts[0]-nav[j+17*nnav];
			if (dt<-MAXDTOE||MAXDTOE<dt) continue;
			if ((svh=(int)nav[j+30*nnav])) break;
			if (ABS(dt)<ABS(dtmin)) {dtmin=dt; imin=j;}
		}
		if (svh||imin<0) {
			if ((int)*dflg>=1) {
			    mexPrintf("warning : %s ts=%.0f,sat=%2.0f svh=%d\n",
						  svh?"SV health bad":"no nav message",*ts,iz[i],svh);
			}
			vz[i]=0;
			continue;
		}
		...
		/* tropospheric delay */
		if ((int)*opt==0) {
			trop_saast(td,azel,gpos,NULL,&trop,NULL);
		}
		/* residual */
						...
						const double *opt, double *x)
{
	...
	for (i=0;i<10;i++) {
		/* residulas */
		if ((n=residual(td,ts,z,iz,vz,nz,nav,inav,nnav,x,x+3,dflg,opt,dz,H))
				<MINOBS) break;
					 ...
					 double *posr, double *cdtr)
{
	...
	while (1) {
		...
		if ((rms=AdjustPos(td,ts,z,iz,vz,nz,nav,inav,nnav,dflg,opt,x))<0.0) return;
		if (rms<MAXRMS) break;
		...
		for (i=n=0,minr=999999999.9;i<nz;i++) {
			...
			if ((r=AdjustPos(td,ts,z,iz,vz,nz,nav,inav,nnav,dflg,opt,x))<minr) {
				minr=r; n=i;
			}
			vz[i]=1;
		}
