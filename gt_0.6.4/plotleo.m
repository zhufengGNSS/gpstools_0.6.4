...
%            08/12/15  0.2  add reference ephemeris parameter
%-------------------------------------------------------------------------------
...
sel4={{'GRACE RL00','GRACE RL01','CHAMP'},{'GRA00','GRA01','CHAMP'}};
...
gut('newdlg','','Read Data',[314,319]);
gut('newprms','prm1',[12,114,295,25,118],prm1);
gut('newprms','prm2',[12,34,295,18,118],prm2);
gut('newokcancelbtn','',[127,4,180,23]);
gut('setprms','prm1',prm.tstart,prm.tend,prm.tint,prm.tunit,prm.sat,prm.fb,...
    prm.refeph,prm.interp);
if ~gut('waitok'), return, end
[prm.tstart,prm.tend,prm.tint,prm.tunit,prm.sat,prm.fb,prm.refeph,prm.interp]=...
 gut('getprms','prm1');
...
switch prm.refeph
case {'GRA00','GRA01'}
    tu=86400; if strcmp(prm.refeph,'GRA00'), ver=0; else ver=1; end
    for ts=(floor(prm.time(1)/tu):floor(prm.time(end)/tu))*tu
        ep=mjdtocal(prm.td+ts/86400);
        file=sprintf('GNV1B_%04d-%02d-%02d_%s_%02d.dat',ep(1:3),prm.sat(end),ver);
