...
%            iz = obervation data index [ttag,isat,ircv,arcf,trcv;...]
%                 ttag = observation time in day (sec)
%                 isat = satellite number
%                 ircv = station number
%                 arcf = arc flag (0=na/arc cont,1=arc start,2=arc end)
%                 trcv = reception time in day (sec)
...
%            06/12/14   0.9  fix bug reading data from obsdir including %S %s
%            08/06/05   0.10 support prn32-99 for rinex obs (gt_0.6.3p5)
%            08/12/02   0.11 add trcv in iz(:,5) in argout
%-------------------------------------------------------------------------------
...
if ~isempty(data)
    ...
    sats={}; for i=1:99, sats={sats{:},sprintf('GPS%02d',i)}; end % support PRN32-99
end
