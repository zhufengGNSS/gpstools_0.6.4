...
%            08/11/21  0.2  support itrf2005,igs05 (gt_0.6.4)
%                           support output position options
%                           support error bar plot
%                           output positions after bandpass filtering
%-------------------------------------------------------------------------------
...
sel1={{'Estimated(Forward)','Estimated(Backward)','Estimated(Smoothed)',...
       ...
       'IGS Final','ITRF2005','ITRF2000','ITRF97','IGS00','IGb00','IGS05','GSIPOS'},...
      {'posf','posb','posfb','possf','possb','possfb',...
       'igssnx','itrf2005','itrf2000','itrf97','igs00','igb00','igs05','gsipos'}};
sel2={{'Average','Linear Fit','Poly Fit','Start','End','Est Average',...
       'IGS Final','ITRF2005','ITRF2000','ITRF97','IGS00','IGb00','IGS05','GSIPOS'},...
      {'estm','estf','estp','ests','este','est0',...
       'igssnx','itrf2005','itrf2000','itrf97','igs00','igb00','igs05','gsipos'}};
sel3={{'','ITRF2005','ITRF2000','ITRF97','IGS00','IGb00','IGS05'},...
      {'','itrf2005','itrf2000','itrf97','igs00','igb00','igs05'}};
sel4={{'OFF','ON'},0:1};
...
data=get(gcf,'userdata'); prm=data.prm;
prm1={
'p','Time Format',{{'YYYY/MM/DD HH:mm:ss','YYYY MM DD TOD','Week TOW'},1:3};
'p','Position Format',{{'ECEF-X/Y/Z','Lat/Long/Height','Error-X/Y/Z','Error-E/N/U','RTKLIB Format'},1:5};
'p','Output Header',{{'ON','OFF'},[1,0]}
'p','Field Separator',{{',',' '},{',',' '}}
};
gut('newdlg','','Output Position',[274,140]);
gut('newprms','prm1',[12,34,255,25,140],prm1);
gut('newokcancelbtn','',[88,4,180,23]);
if ~isempty(opt), gut('setprms','prm1',opt(1),opt(2),opt(3),opt(4)); end
if ~gut('waitok'), return, end
[tform,pform,outhead,sep]=gut('getprms','prm1');
opt=[tform,pform,outhead,sep];
close
i=find(strcmp(prm.rcv,prm.rcvs));
if isempty(data.pos)|isempty(i), return, end
pos=data.pos{i}(:,1:3); err=data.err{i}; sig=data.sig{i};
rcv=RcvName(prm.rcvs{i},prm.showf(4));
switch pform
case 1, [s1,s2]=outputxyz(rcv,pos,sig(:,2:4),prm,tform,sep);
case 2, [s1,s2]=outputllh(rcv,pos,sig(:,5:7),prm,tform,sep);
case 3, [s1,s2]=outputerrxyz(rcv,err(:,2:4),sig(:,2:4),prm,tform,sep);
case 4, [s1,s2]=outputerrenu(rcv,err(:,5:7),sig(:,5:7),prm,tform,sep);
case 5, [s1,s2]=outputrtklib(rcv,pos,sig(:,5:7),prm);
end
if outhead, s={s1{:},s2{:}}; else s=s2; end
Viewer(s,['Receiver Position : ',prm.dirs.est,' (',prm.fb,')']);

% output position in ecef-xyz --------------------------------------------------
function [s1,s2]=outputxyz(rcv,pos,sig,prm,tform,sep)
ts={'  Date       Time     ','  Date     TOD(sec) ',' Week  TOW(sec) '};
s1={};
s1=adds(s1,'%%  RECEIVER POSITION: %s : %s-%s GPST',rcv,tstr(prm.td,prm.time(1)),tstr(prm.td,prm.time(end)));
s1=adds(s1,['%%',ts{tform},'       X(m)           Y(m)           Z(m)       SdX(m)  SdY(m)  SdZ(m)']);
s1=adds(s1,'%%------------------------------------------------------------------------------------------');
s2=cell(1,length(prm.time));
for n=1:length(prm.time)
    s2{n}=sprintf('%s%s %14.4f%s%14.4f%s%14.4f%s %7.4f%s%7.4f%s%7.4f',...
                  timef(prm.td,prm.time(n),tform,sep),sep,pos(n,1),sep,...
                  pos(n,2),sep,pos(n,3),sep,sig(n,1),sep,sig(n,2),sep,sig(n,3));
end

% output position in lat/lon/hgt -----------------------------------------------
function [s1,s2]=outputllh(rcv,pos,sig,prm,tform,sep)
ts={'  Date       Time     ','  Date     TOD(sec) ',' Week  TOW(sec) '};
s1={};
s1=adds(s1,'%%  RECEIVER POSITION: %s : %s-%s GPST',rcv,tstr(prm.td,prm.time(1)),tstr(prm.td,prm.time(end)));
s1=adds(s1,['%%',ts{tform},'   Latitude(deg)   Longitude(deg)   Height(m)   SdN(m)  SdE(m)  SdU(m)']);
s1=adds(s1,'%%------------------------------------------------------------------------------------------');
s2=cell(1,length(prm.time));
for n=1:length(prm.time)
    posr=eceftogeod(pos(n,:)');
    s2{n}=sprintf('%s%s %15.10f%s%16.10f%s%11.4f%s %7.4f%s%7.4f%s%7.4f',...
                  timef(prm.td,prm.time(n),tform,sep),sep,posr(1),sep,...
                  posr(2),sep,posr(3),sep,sig(n,2),sep,sig(n,1),sep,sig(n,3));
end

% output position in error-x/y/z -----------------------------------------------
function [s1,s2]=outputerrxyz(rcv,err,sig,prm,tform,sep)
ts={'  Date       Time     ','  Date     TOD(sec) ',' Week  TOW(sec) '};
s1={};
s1=adds(s1,'%%  RECEIVER POSITION ERROR: %s : %s-%s GPST',rcv,tstr(prm.td,prm.time(1)),tstr(prm.td,prm.time(end)));
s1=adds(s1,['%%',ts{tform},'        X(m)        Y(m)        Z(m)   SdX(m)  SdY(m)  SdZ(m)']);
s1=adds(s1,'%%------------------------------------------------------------------------------------------');
s2=cell(1,length(prm.time));
for n=1:length(prm.time)
    s2{n}=sprintf('%s%s %11.4f%s%11.4f%s%11.4f%s %7.4f%s%7.4f%s%7.4f',...
                  timef(prm.td,prm.time(n),tform,sep),sep,err(n,1),sep,...
                  err(n,2),sep,err(n,3),sep,sig(n,1),sep,sig(n,2),sep,sig(n,3));
end

% output position in error-e/n/u -----------------------------------------------
function [s1,s2]=outputerrenu(rcv,err,sig,prm,tform,sep)
ts={'  Date       Time     ','  Date     TOD(sec) ',' Week  TOW(sec) '};
s1={};
s1=adds(s1,'%%  RECEIVER POSITION ERROR: %s : %s-%s GPST',rcv,tstr(prm.td,prm.time(1)),tstr(prm.td,prm.time(end)));
s1=adds(s1,['%%',ts{tform},'     East(m)    North(m)       Up(m)   SdE(m)  SdN(m)  SdU(m)']);
s1=adds(s1,'%%------------------------------------------------------------------------------------------');
s2=cell(1,length(prm.time));
for n=1:length(prm.time)
    s2{n}=sprintf('%s%s %11.4f%s%11.4f%s%11.4f%s %7.4f%s%7.4f%s%7.4f',...
...
% output position in rtklib format ---------------------------------------------
function [s1,s2]=outputrtklib(rcv,pos,sig,prm)
s1={};
s1=adds(s1,'%% program   : GpsTools ver.%s',gpstools('version'));
s1=adds(s1,'%% receiver  : %s',rcv);
s1=adds(s1,'%% directory : %s',prm.dirs.est);
s1=adds(s1,'%% (lat/lon/height=WGS84/ellipsoidal,Q=1:fix,2:float,4:dgps,5:single,ns=# of satellites)');
s1=adds(s1,'%%  GPST                  latitude(deg) longitude(deg)  height(m)   Q  ns   sdn(m)   sde(m)   sdu(m)');
s2={}; m=0;
for n=1:length(prm.time)
    posr=eceftogeod(pos(n,:)'); dv=mjdtocal(prm.td,prm.time(n));
    if ~isnan(posr(1))
        m=m+1;
        s2{m}=sprintf('%04d/%02d/%02d %02d:%02d:%06.3f %14.9f %14.9f %10.4f %3d %3d %8.4f %8.4f %8.4f',...
                      dv, posr,2,0,sig(n,[2,1,3]));
    end
end

% time format ------------------------------------------------------------------
function s=timef(td,time,tform,sep)
switch tform
case 1
    dv=mjdtocal(td,time);
    s=sprintf('%04d/%02d/%02d%s%02d:%02d:%05.2f',dv(1:3),sep,dv(4:6));
case 2
    td=td+floor(time/86400); tod=mod(time,86400); d=mjdtocal(td);
    s=sprintf('%04d%s%02d%s%02d%s%9.2f',d(1),sep,d(2),sep,d(3),sep,tod);
case 3
    td=td-caltomjd([1980,1,6])+time/86400; week=floor(td/7); tow=(td-week*7)*86400;
    s=sprintf('%5d%s%10.2f',week,sep,tow);
end
...
data=get(gcf,'userdata'); prm=data.prm;
sel1={{'OFF','ON'},0:1};
sel2={{'Dot','Line','Line and Dot','Circle and Error Bar'},1:4};
...
for n=1:3
    ...
    plotval(time,err(:,n+1),sig(:,n+1)*prm.nsig,prm.ptype,prm.psize);
    s1=''; s2='';
    if prm.showf(1)
...
for n=1:3
    ...
    ggt('subplotv','err',n,3,'move',[1,1,1,1],'link',[1,0,1,0],...
        'taxis',prm.td,'topts',topts,'margin',margin,...
        'xlim',xl,'ylim',prm.range);
    plotval(time,err(:,n+4),sig(:,n+4)*prm.nsig,prm.ptype,prm.psize);
    s1=''; s2='';
    if prm.showf(1)
...
for n=1:length(prm.rcvs)
    ...
    if prm.bpfilt(1)<prm.bpfilt(2) % bandpass filter
        i=find(~isnan(poss(:,1,n)));
        if length(i)>=2
            off=repmat(mean(poss(i,:,n),1),size(poss,1),1);
            poso=poss(:,:,n)-off;
            poso=bpfilt(prm.time,poso,prm.bpfilt(1),prm.bpfilt(2));
            poss(:,:,n)=poso+off;
        end
    end
end
...
function [err,sig]=ErrEstPos(td,time,rcvs,dirs,pos,var,posr,tunit)
err={}; sig={};
for n=1:length(rcvs)
...
strs={'Average','Linear Fit','Poly Fit','Start Pos','End Pos','Est Average','ITRF2005','ITRF2000','ITRF97','IGS Final','GSIPOS','IGS00','IGb00','IGS05'};
refs={'estm','estf','estp','ests','este','est0','itrf2005','itrf2000','itrf97','igssnx','gsipos','igs00','igb00','igs05'};
...
% plot value -------------------------------------------------------------------
function h=plotval(t,x,sig,ptype,psize)
if ptype==1|ptype==3, h=plot(t/3600,x,'.','markersize',psize(2)); end
if ptype==2|ptype==3, h=plot(t/3600,x,'-','linewidth',psize(1)); end
if ptype==4
    for i=1:length(t)
        plot(t(i)/3600+[0 0],x(i)+[1 -1]*sig(i),'.-','markersize',3);
    end
    h=plot(t/3600,x,'o','markersize',psize(2));
end
